...
- **อ้างอิง** — 42 รายการ · เปิดอ่านเต็มทุกหน้า
...
| 42 | **Graph view** · Obsidian `[OBSIDIAN]`<br>node เป็นวงกลมที่ขนาดแปรตามจำนวนการอ้างถึง, ค่า Text fade threshold, และแรงสี่ตัวที่ปรับได้คือ center force · repel force · link force · link distance<br><https://obsidian.md/help/plugins/graph> |
...
| --- | --- | --- |
| node | *“Nodes are used to represent entities (discrete objects) of a domain.”* | 60 วงกลมในภาพ ขนาดของวงบอกจำนวนความสัมพันธ์ที่ต่อกับมัน |
...
ชี้ที่กล่องเพื่อดูว่ามันเชื่อมกับอะไรบ้างด้วยความสัมพันธ์ชื่ออะไร · คลิกเพื่ออ่านคำอธิบายเต็ม · ชี้ที่เส้นเพื่ออ่านทีละเส้น

ในฉบับเว็บแบ่งเป็นสองจังหวะ **ชี้** ที่กล่องแล้วกล่องที่ไม่เกี่ยวจะจางลง เหลือเฉพาะ node นั้นกับเพื่อนบ้านของมัน พร้อมชื่อความสัมพันธ์ทุกเส้นโผล่มาพร้อมกัน ตอบคำถามว่าอะไรเชื่อมกับอะไรได้ในทีเดียวโดยไม่ต้องกด · **คลิก** ถึงจะเปิดหน้าต่างซ้อนขึ้นมาตรงกลางจอ แสดงคำอธิบายว่า node นั้นคืออะไร คุณสมบัติของมัน และความสัมพันธ์ทุกเส้นที่ต่อกับมัน แยกเป็นสองกลุ่มคือสิ่งที่มันทำกับตัวอื่น และสิ่งที่ตัวอื่นทำกับมัน

ตำแหน่งป้ายชื่อความสัมพันธ์คำนวณไว้ล่วงหน้าโดยมีเงื่อนไขเพิ่มอีกข้อ คือป้ายของเส้นที่ออกจาก node เดียวกันต้องไม่ทับกันเอง เพราะป้ายชุดนั้นถูกแสดงพร้อมกันตอนชี้

> **รูปที่ 16 — Knowledge Graph ของเอกสารทั้งฉบับ** · 60 node และ 76 relationship แบ่งด้วยสีตามหมวด node วาดเป็นวงกลมที่ขนาดแปรตามจำนวนความสัมพันธ์ที่ต่อกับมัน ตำแหน่งคำนวณด้วย force layout แบบ deterministic นอกเบราว์เซอร์ แล้วตรึงพิกัดไว้ จึงตรวจได้ว่าไม่มีวงทับกันและไม่มีเส้นลอดผ่านวงใด ชื่อ relationship ไม่ถูกวาดค้างไว้บนภาพ ให้ชี้ที่ node เพื่ออ่านทั้งชุดพร้อมกัน หรือชี้ที่เส้นเพื่ออ่านทีละเส้น
...
| วงของ node ทับกัน | ข้อกำหนดการแสดงผลของเอกสารนี้ | 0 คู่ |
| เส้นลอดผ่านวงที่ไม่ใช่ปลายทางตัวเอง | ทำให้อ่านผิดว่าเส้นนั้นเชื่อมกับ node ที่มันลอดผ่าน | 0 จุด |
| ชื่อ node ทับวงหรือทับชื่ออื่น | ชื่ออยู่นอกวง จึงต้องหาที่ว่างให้ครบทุกตัว | 0 จาก 60 |

> **หมายเหตุ — ทำไมไม่เขียนชื่อ relationship ค้างไว้บนภาพ**
>
> ลองวางป้ายชื่อทั้ง 76 เส้นค้างไว้พร้อมกันแล้วตรวจดู พบว่ามีป้ายทับกันเอง ซึ่งขัดกับข้อกำหนดของเอกสารนี้ จึงเปลี่ยนเป็นคำนวณตำแหน่งที่ว่างของแต่ละเส้นไว้ล่วงหน้า แล้วแสดงเฉพาะชุดที่เกี่ยวข้องตอนชี้ที่ node ส่วนภาพรวมของความสัมพันธ์ทั้งหมดอ่านได้จากตาราง triple ด้านบน

### ทำไม node เป็นวงกลม ไม่ใช่กล่องข้อความ

รุ่นก่อนหน้าวาด node เป็นกล่องที่กว้างตามความยาวชื่อ กล่องที่กว้างสุดคือ `SpringBootServletInitializer` ที่ 244px ผลคือพื้นที่กีดขวางในภาพมหาศาล และเงื่อนไข “ห้ามมีเส้นพาดผ่านกล่อง” บังคับให้ผังต้องกระจายแบบ organic เท่านั้น ลองจัดเป็นคอลัมน์แล้วได้เส้นพาดผ่านกล่องถึง 112 เส้น

graph view ของ Obsidian แก้ปัญหานี้ด้วยการไม่ใช้กล่องตั้งแต่แรก เอกสารระบุว่า node เป็นวงกลมที่ **“The more nodes that reference a given node, the bigger it gets”**`[OBSIDIAN]` เมื่อ node กินพื้นที่แค่วงรัศมี 7 ถึง 19px ปัญหาเส้นชนก็เกือบหายไปเอง และได้ลำดับความสำคัญมาฟรี — `w3wp.exe` ที่มี 8 ความสัมพันธ์เด่นกว่า `mimeMap` ที่มี 1 ทันทีโดยไม่ต้องอธิบาย

| | รุ่นกล่อง | รุ่นวงกลม |
| --- | --- | --- |
| ขนาด node | กว้าง 51–244px สูง 32px | รัศมี 6.8–19px |
| ขนาดภาพรวม | 1414 × 1333 | 1275 × 1059 |
| ความยาวเส้นเฉลี่ย | 278px | 214px |
| เส้นตัดกันเอง | 81 คู่ | 82 คู่ |

ชื่อ node ย้ายออกมาอยู่นอกวง และจางหายเมื่อซูมออก ตามแนวคิด **Text fade threshold** ของ Obsidian ที่ *“controls the text transparency for the name of each note”* ยกเว้น node ที่กำลังถูกชี้กับเพื่อนบ้านของมัน ซึ่งอ่านชื่อได้เสมอไม่ว่าซูมระดับใด
